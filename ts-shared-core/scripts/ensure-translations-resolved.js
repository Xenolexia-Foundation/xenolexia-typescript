...
function fetchOne(langCode) {
  ...
  try {
    ...
    console.error(
      `[ensure-translations-resolved] Fetch failed for ${langCode} (likely rate limit).`
    );
    throw e;
  }
}
...
function main() {
  ...
  console.log(
    `[ensure-translations-resolved] Languages with MyMemory warning: ${needsFix.join(', ')}`
  );
  ...
  try {
    ...
    console.error(
      '[ensure-translations-resolved] Run failed (translation service may have returned rate-limit warning). Exit 1.'
    );
    process.exit(1);
  }
  ...
  if (remaining.length > 0) {
    console.log(
      `[ensure-translations-resolved] Remaining: ${remaining.join(', ')}. Run again in 4 hours.`
    );
  } else {
